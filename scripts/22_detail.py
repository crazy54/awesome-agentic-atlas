...
/* --ui and --wash are the structural tokens declared in the block above; graphite resolves them to this
   ...
   before. The 1.6 line height is this page's own and stays -- it is prose, not a table. */
body{margin:0;background:var(--surface);background-image:var(--wash);
  background-attachment:fixed;color:var(--ink);font:15px/1.6 var(--ui)}
...
try {
  ...
  // The sixteen literals track --plane in the sixteen blocks at the top of detail.css. Once detail.js has run
  // it re-derives this from the computed value so the stylesheet stays the single source of truth; here
  ...
  // work for the choice to mean anything -- a reader who picks Prism and follows a link to a project would
  // otherwise watch it revert and conclude the setting had not saved. Unknown names fall back to graphite
  // for the reason the index's copy gives: `atlas-skin` outlives the release that wrote it.
