...
runs eighteen harnesses in turn, prints what each one asserted, and exits non-zero if anything failed. About
...
There are eighteen files rather than one because they are eighteen instruments, and the overlap between them is the
...
Between them, 2,581 assertions, from a real run rather than by adding up the figures above. The number only
