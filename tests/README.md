...
known-red list and a quicker reference.
...
Leave that running, and in another shell (in `run.mjs` order):
...
python tests/deeplinks_test.py
python tests/spotlight_test.py
...
The nine red ones were red on `latest_branch` `a77b46eb` before any documentation change, and each is named
with its cause, where known, in [the handbook](../handbook/testing.md#known-red-and-environment-sensitive-checks):
one in `probe.mjs`, one in `semantic_test.py`, one in `detail-preview-check.mjs`, two in `pwa-check.mjs` (the
worker's `VERSION` is stale), two in `dance-check.mjs`, one in `friends-check.mjs` and one in `rig-check.mjs`.
Re-run the suite to update this paragraph; do not adjust the numbers by hand.

One blind spot is structural. `probe.mjs` reads the committed `docs/catalog/index.html`, which only
`19_pages.py` can rebuild, and that stage needs the crawl cache CI has and a checkout does not. So a change
to the catalogue's source is green here and unmeasured by `probe.mjs` until a build lands, and the whole of
`theme_test.py` exists on the other side of that line, reading `scripts/19_pages.py` as text precisely so it
does not have to wait for one. A green run on a checkout means every harness agrees with the tree; it does
not mean the published page has caught up with it. See `probe.mjs`'s own header, `scripts/devfixture.py`
for the local render that closes most of the gap, and `19b_refresh.py` for why the other local render path
refuses when the committed rows and the configured source lists disagree.
...
- `node:http` is all a static server needs. `lib/serve.mjs` is 121 lines including its comments.
...
plumbing — 384 lines of it, in `lib/` (2026-09-25). That is the trade, and it is deliberate.

The sixteen Python harnesses are Python because the things they test are. `python` here means whatever
...
`refresh_test.py`, `live_test.py`, `collections_test.py`, `deeplinks_test.py`, `spotlight_test.py`, `osicons_test.py` and `semantic_test.py` — standard
...
- exits 2 before running anything if no Chromium or no Python 3 is found, with the list of places it looked and
  the two cheapest fixes, or if `docs/index.html` or `docs/catalog/index.html` is missing. A missing prerequisite stops the run; it never reduces it.
...
- **The build bots' commits do not trigger it.** `daily.yml` and `weekly.yml` push with `secrets.GITHUB_TOKEN`,
  and GitHub starts no workflow runs for pushes made with that token. Both builds therefore run
  `node tests/run.mjs` themselves, against the tree they are about to commit. The weekly's red suite blocks
  its commit (`VERIFY_GATES_DEPLOY: "true"`); the daily's does not (`"false"`): it publishes and then ends
  red. See [the handbook's CI page](../handbook/ci-cd.md).
- **Exit 2 means the suite never ran.** The workflow annotates 1 and 2 differently on purpose. 1 is this
  ...
  because it found no Chromium, no Python 3, or no built `docs/index.html` / `docs/catalog/index.html`, which is a fact about the runner and not about the code, and
...
| `AAA_PAGE` | Point `probe.mjs`'s text scans at a different HTML file (default `docs/catalog/index.html`). A development hatch for working on `pagemin.py`; the page script cannot boot from an unsubstituted template. |
| `AAA_DATA` | `probe.mjs` only: read a different `data.json` instead of `docs/data.json`. |
| `AAA_HOME` | `probe.mjs` only: read a different homepage instead of `docs/index.html`. |
| `PLAYWRIGHT_BROWSERS_PATH` | An extra Playwright browser cache to search before the default ones. |
...
A test suite nobody has watched fail is not known to work. Six cheap ways to check this one still bites:

- Give a template a timestamp. Add `datetime.now()` to anything `scripts/22_detail.py` renders and
  `detail-churn.mjs` reports `two runs over identical data write identical bytes -- N file(s)`, then
  restore it.
- Break a scoping rule. Copy `docs/catalog/index.html`, delete an `html[data-view=cards]` prefix from one selector in
...
- Make the view switch rebuild the list. Add a `render()` call to the `#view` button's `onclick` in
  `docs/catalog/index.html` — the regression the toggle exists to avoid — and `cards-check.mjs` prints
