...
Between them, 3,136 assertions, from a real run rather than by adding up the figures above. The number only
matters in one direction — see the floors below. Seventeen are red on the tree this sentence was written on,
and they are worth naming rather than leaving a total to imply otherwise, because a reader who finds a red
suite and no account of it cannot tell which failures are expected: eleven in `cards-check.mjs` are the
instrument rather than the page — CDP's synthetic pointer does not land on a row, which reproduces under
`chrome-headless-shell` on a workstation as well as on a runner, so it is not runner flakiness; five are a
Discover day publishing forty-nine picks under a heading that promises fifty, which is a real defect being
fixed on its own branch; and one is `semantic_test.py`'s map extent. None of them is a claim about the
palette, and none was introduced here — they were counted on the base commit before any of this landed,
which is the only way that sentence is worth anything. The two that were red here for months were
