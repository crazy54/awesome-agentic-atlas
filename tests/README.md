...
Between them, 2,789 assertions, from a real run rather than by adding up the figures above. The number only
matters in one direction — see the floors below. Two of them are red on the tree this sentence was written
on, both `probe.mjs`'s: they read a `docs/index.html` that `19_pages.py` alone can rebuild, and that stage
needs the crawl cache CI has and a checkout does not. That is the standing shape of this suite's one blind
spot rather than a defect in it — see `probe.mjs`'s own header, and `19b_refresh.py` for why the local
render path refuses when the committed rows and the configured source lists disagree.
