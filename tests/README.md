...
Between them, 3,133 assertions, from a real run rather than by adding up the figures above. The number only
matters in one direction — see the floors below. All of them are green on the tree this sentence was written
on, which is worth stating precisely rather than leaving as reassurance: the two that were red here for
months were `probe.mjs`'s, and they were red because it reads a `docs/index.html` that `19_pages.py` alone
