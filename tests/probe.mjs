...
const shim = [
  ...
  // The export. `HITS` is read-only for the same reason `SAVED` is: it is what `render()` last drew, and a
  // test that assigned it would be asserting that four documents can be built from a list -- which is not
  // in doubt -- instead of that they describe the table the reader was looking at. `download()` is not
  // exposed at all, because Blob, object URLs and `document.body` are all absent here; everything it
  // writes is generated by the four pure functions above it, which are.
  "  get HITS(){return HITS}, viewTitle, shareURL, mdCell, osText, hasCmds, listMarkdown, listHTML,",
  "  VERDICT,",
  "  fileName, paintShared, expOpen, printOn, printOff, get PRINT(){return PRINT},",
...
ok("the last-deployed badge is a crisp Shieldcn SVG with a clock icon and semantic time fallback",
   deployed.includes('class="deploy-badge"') && deployed.includes('src="https://shieldcn.dev/badge/') &&
   deployed.includes("logo=ri%3ALuClock3") && deployed.includes("<time ") &&
   deployed.includes("Last deployed"), deployed.slice(0, 320));
ok("the Shieldcn deployment badge stays below the old 23px treatment",
   /height:22px/.test(stampRule), stampRule);
const readmeBadges = [...readme.matchAll(/!\[[^\]]*\]\((https?:\/\/[^)]+)\)/g)].map(m => m[1]);
ok("every README badge uses Shieldcn SVG output",
   readmeBadges.length === 6 && readmeBadges.every(u => u.startsWith("https://shieldcn.dev/") &&
     u.includes(".svg")), readmeBadges.join("\n"));
// The leaderboard badge sits under the surface links, not in the stat row above them: the five stats
// describe the atlas, this one is a way in. It points at the same file the web masthead links to, so a
// reader who arrives from either surface lands on the same ranking.
const lbBadge = readme.match(/\[!\[Atlas Leaderboard\]\((https?:\/\/[^)]+)\)\]\(([^)]+)\)/);
ok("the README offers an Atlas Leaderboard badge that links to the leaderboard",
...
const sortMarkup = html.match(/<select id="sort"[\s\S]*?<\/select>/)?.[0] || "";
const menu = [...sortMarkup.matchAll(/<option value="([^"]*)"/g)].map(m => m[1]);
...
// --------------------------------------------------------------------- take it with you: export and share
// Four documents and one link, all four assembled from `HITS` -- the rows `render()` last put on screen.
// That is the whole of what makes an export trustworthy, and it is the one thing a browser test cannot see:
// Chrome will happily show you a downloaded table without telling you it disagrees with the page behind it.
//
// The assertions are weighted towards the seams rather than the formatting. A missing pipe in a Markdown
// header is visible the first time anyone looks; a `#list=` link that percent-encodes its slashes, an export
// that stops at 120 rows because the page did, or a printed sheet of white-on-white are all things that look
// finished from here and fail on someone else's machine, which is the only machine an export ever runs on.
const XCH = html.match(/<button class="chip takechip" id="take"[^>]*>/)?.[0] || "";
ok("the Export control announces that it opens a dialog", XCH.includes('aria-haspopup="dialog"'), XCH);
// Not a toggle, so not `aria-pressed`. A chip that reports a pressed state it does not have tells a screen
// reader the view has changed when all that happened is that a dialog opened -- and the other chips on this
// row genuinely are toggles, which is exactly how this one would come to be written as one.
ok("...and does not claim to be a toggle like the chips beside it", !XCH.includes("aria-pressed"), XCH);
ok("...and is hidden by the stylesheet until it is wired", /\.takechip\{display:none\}/.test(html));
ok("...then switched on by expWire once the browser can open a modal",
   document.getElementById("take").classList.contains("on"));
ok("the export dialog is labelled by its own heading",
...
ok("printing gets a page box rather than the browser's default margins", /@page\{margin:/.test(html));
...
ok("...and reveals the link that says where the sheet came from",
   /#printurl\{display:block/.test(printCSS) && /#printurl\{display:none\}/.test(html), printCSS);
// The column proportions, which are the difference between a document and a curiosity: the auto layout gave
// paper the proportions of a 900px window, put the description in a 130px column and printed 278 rows over
// ninety sheets at two and a bit rows each. Asserted as a sum because that is the failure a fixed layout
// actually has -- widths that do not add to the table are silently redistributed, and the column that loses
// is whichever one is left over.
ok("...and lays the table out on declared columns rather than on its content",
   /#out table\{table-layout:fixed\}/.test(printCSS), printCSS);
const widths = [...printCSS.matchAll(/^\s*th[^{]*\{width:([\d.]+)%\}/gm)].map(m => parseFloat(m[1]));
ok("...over six columns", widths.length === 6, widths.join("+"));
ok("...whose widths account for the whole sheet",
   Math.abs(widths.reduce((a, b) => a + b, 0) - 100) < 0.01, widths.join("+"));
// Two rules that each fixed a measured defect and neither of which reads as necessary. `.st` carries its own
// 16px, which the 11px body rule cannot reach, and at 16px bold a seven-digit star count printed as "281,17"
// with the 6 beneath it. And `.tag` is `nowrap` so a pill never splits mid-name -- generated content inherits
// that, and a nowrap space is not a break opportunity, so moving the separator to `::after` without this made
// the whole cell one unbreakable line that ran through the next column and gave the table a scrollbar.
ok("...and takes the star count out of screen type so it fits its column",
   /\.st\{font-size:1[12]px\}/.test(printCSS), printCSS);
ok("...and flattens the chips into text", /td\.tg \.tag\{display:inline;/.test(printCSS), printCSS);
ok("...with a separator that can be broken after",
   /td\.tg \.tag:not\(:last-child\)::after\{content:", ";white-space:normal\}/.test(printCSS), printCSS);

// --- a #list= link travels: it means the same rows on any machine, which #saved=1 cannot ---
const rowA = A.ROWS[0];
const rowB = A.ROWS.find(r => r.cat !== rowA.cat);
const pair = [rowA.nwo, rowB.nwo];
globalThis.location.hash = "#list=" + pair.join(",");
A.readHash();
ok("a #list= link is read back as a set of projects", A.state.list.size === 2, [...A.state.list].join(","));
A.state.shown = 600; A.render();
ok("...and the page shows exactly those projects", order().length === 2 &&
   pair.every(n => order().includes(n)), order().join(","));
ok("...and the set survives the count-with-one-filter-lifted passes render makes",
   A.state.list.size === 2, String(A.state.list.size));
// The reason this parameter is appended by hand instead of going through URLSearchParams. A link somebody is
// about to paste into a message has to be readable enough to trust before clicking, and `%2F` sixty times
// over is not; both characters are legal unescaped in a fragment.
let wrote = "";
const realRS = globalThis.history.replaceState;
globalThis.history.replaceState = (a, b, u) => { wrote = String(u); };
A.writeHash();
globalThis.history.replaceState = realRS;
ok("the shared link keeps its slashes and commas unescaped",
   wrote.endsWith("list=" + pair.join(",")) && !/%2F|%2C/i.test(wrote), wrote);
// Composes with the rest, like every other filter: a shared list crossed with a topic is the crossing.
A.state.cat = A.D.cats[rowA.cat].slug;
A.state.shown = 600; A.render();
ok("a shared list narrows further when another filter is on", order().length === 1 &&
   order()[0] === rowA.nwo, order().join(","));
// And when the crossing is empty, the way out is offered -- the shared list is the filter a reader is least
// likely to guess is the one excluding everything, because they did not set it.
A.state.cat = A.D.cats.find(c => c.slug !== A.D.cats[rowA.cat].slug).slug;
A.state.list = new Set([rowA.nwo]);
A.state.shown = 600; A.render();
if (order().length === 0) {
  const rl = A.rescue();
  ok("an empty shared-list-and-topic view offers to look beyond the list",
     rl.some(o => /shared list/.test(o.label)), rl.map(o => o.label).join(" | "));
  ok("...by emptying the list and nothing else",
     rl.some(o => o.patch.list instanceof Set && o.patch.list.size === 0),
     JSON.stringify(rl.map(o => Object.keys(o.patch))));
}

// --- the strip a shared list arrives under, and the two ways out of it ---
A.state.cat = ""; A.state.q = "";
STORE.clear(); A.SAVED.clear();
A.state.list = new Set(pair);
A.state.shown = 600; A.render();
ok("the shared-list strip is shown", G2("shared").classList.contains("on"));
ok("...and says how many projects, in prose rather than a count badge",
   /shared list of <b>2 projects<\/b>/.test(G2("sharedtext").innerHTML), G2("sharedtext").innerHTML);
ok("...and offers to make the list the reader's own", G2("sharedadd").disabled === false &&
   /Save 2 to my projects/.test(G2("sharedadd").textContent), G2("sharedadd").textContent);
G2("sharedadd").click();
ok("pressing it saves the whole shared list", pair.every(n => A.SAVED.has(n)), [...A.SAVED].join(","));
ok("...and it reaches storage, which is the only thing that makes the list durable",
   JSON.parse(STORE.get("saved") || "[]").length === 2, STORE.get("saved"));
ok("...and does not throw the reader back to the top of the list",
   A.state.shown === 600, String(A.state.shown));
// Disabled rather than removed, so the row does not reflow under the cursor between two presses.
ok("...then reports that there is nothing left to add, in place",
   G2("sharedadd").disabled === true && /All saved/.test(G2("sharedadd").textContent),
   G2("sharedadd").textContent);
G2("sharedall").click();
ok("the other way out shows the whole atlas again", A.state.list.size === 0 && order().length > 2,
   order().length + " rows");
ok("...and takes the strip with it", !G2("shared").classList.contains("on"));
A.state.list = new Set([rowA.nwo]); A.state.shown = 600; A.render();
G2("reset").click();
ok("Clear all filters drops the shared list too", A.state.list.size === 0, String(A.state.list.size));

// --- the link, which is not always the URL in the address bar ---
STORE.clear(); A.SAVED.clear();
A.state.list = new Set(); A.state.cat = ""; A.state.q = ""; A.state.saved = false;
globalThis.location.href = "https://example.org/atlas/#topic=" + A.D.cats[0].slug;
globalThis.location.hash = "#topic=" + A.D.cats[0].slug;
ok("an ordinary view is shared as the URL the reader can see",
   A.shareURL() === globalThis.location.href, A.shareURL());
A.toggleSave(btn(rowA.nwo, rowA.name));
A.toggleSave(btn(rowB.nwo, rowB.name));
globalThis.location.href = "https://example.org/atlas/#saved=1&sort=stars";
globalThis.location.hash = "#saved=1&sort=stars";
A.state.saved = true; A.state.cat = ""; A.state.sort = "stars"; A.state.shown = 600; A.render();
const shareable = A.shareURL();
ok("a saved view is shared as the collection itself, because #saved=1 means the reader's own set",
   /#list=/.test(shareable) && !/saved=1/.test(shareable), shareable);
ok("...carrying every row that was on screen", pair.every(n => shareable.includes(n)), shareable);
ok("...and keeping the rest of the view intact", /sort=stars/.test(shareable), shareable);

// --- the two documents, which are text and so can be asserted on exactly ---
A.state.saved = false; A.state.sort = "relevance"; A.state.q = "agent";
A.state.shown = 600; A.render();
const md = A.listMarkdown();
const cells = line => line.split(/(?<!\\)\|/).length;
const mdRows = md.split("\n").filter(l => l.startsWith("| "));
ok("the Markdown export is titled with the view, not with the site",
   md.startsWith("# " + A.viewTitle()), md.slice(0, 90));
// A table of star counts with no date on it is wrong within the week, and the two dates are different facts.
ok("...and carries its provenance: the snapshot, the export date and a link back",
   md.includes("Awesome Agentic Atlas](") && /Exported \d{4}-\d\d-\d\d\./.test(md) &&
   md.includes(html.match(/const SNAPSHOT = "([^"]*)"/)[1]), md.split("\n")[2]);
ok("...and is a GitHub-flavoured table with a header and an alignment rule",
   mdRows.length === A.HITS.length + 2 && /^\| *--:/.test(mdRows[1]),
   mdRows.length + " rows for " + A.HITS.length + " hits");
ok("...whose every row has exactly as many cells as the header",
   mdRows.every(l => cells(l) === cells(mdRows[0])),
   [...new Set(mdRows.map(cells))].join("/") + " vs " + cells(mdRows[0]));
ok("...and one row per project on screen, in the order they were drawn",
   mdRows[2].includes("](" + A.HITS[0].url + ")"), mdRows[2].slice(0, 100));
ok("a pipe in a description does not take a column with it", A.mdCell("runs foo | bar") ===
   "runs foo \\| bar", A.mdCell("runs foo | bar"));
ok("...and a wrapped blurb is flattened rather than dropped", A.mdCell("a\n b \tc") === "a b c",
   JSON.stringify(A.mdCell("a\n b \tc")));
...
ok("the platform column is legible without colour",
   ...
   A.osText(A.HITS[0]));
// Five verdicts, five distinct marks. `a` and `-` both drew – in the same colour at the same opacity, so
// "does not apply to this project" and "we could not tell" were the same cell to every reader. Colour is not
// the missing channel here -- there was no channel. Asserted on the map rather than on a rendered row because
// the pair is 220 of 6,470 cells and no row carries both, so sampling rows can miss it entirely.
const marks = Object.values(A.VERDICT).map(v => v[0]);
ok("every verdict has a mark of its own", new Set(marks).size === marks.length, marks.join(" "));
// The legend is a second source for the same fact -- the drift `osicons.py` prevents by generating both ends.
// There is no generator here, so this is the guard: a mark added to `VERDICT` and not to the key on the page,
// or renamed in one place only, fails here rather than shipping a legend that explains four of five marks.
const vkey = html.match(/<details class="vkey">[\s\S]*?<\/details>/);
ok("the page carries a verdict legend", !!vkey);
if (vkey) {
  const pairs = [...vkey[0].matchAll(/<dt [^>]*>([^<]+)<\/dt>\s*<dd>([\s\S]*?)<\/dd>/g)]
    .map(m => [m[1].trim(), m[2].replace(/<[^>]*>/g, "").replace(/\s+/g, " ").trim().toLowerCase()]);
  ok("...with one entry per verdict, and no extras",
     pairs.length === marks.length && marks.every(g => pairs.some(([d]) => d === g)),
     "legend " + pairs.map(([d]) => d).join(" ") + " vs VERDICT " + marks.join(" "));
  // The words as well as the marks. A legend whose glyphs match while its prose has drifted is worse than
  // no legend, because it reads as authoritative; and the prose is the half a reader actually acts on.
  const drifted = Object.values(A.VERDICT).filter(([g, word]) => {
    const pair = pairs.find(([d]) => d === g);
    return !pair || !pair[1].startsWith(word.toLowerCase());
  });
  ok("...each described in the words VERDICT uses", !drifted.length,
     drifted.map(([g, w]) => g + " should read \"" + w + "\"").join("; "));
}
...
// The requirement, not a preference: this file lands in a wiki, an email or a folder on a laptop, and the
// failure mode of a "portable" export that fetches its stylesheet from this origin is that it looks right on
// the machine that made it and broken everywhere else, offline included.
ok("...with nothing to fetch: its CSS is inline and it carries no script, link or image",
   doc.includes("<style>") && !doc.includes("<script") && !doc.includes("<link") &&
   !/ src=/.test(doc));
ok("...and no dependence on a data-theme that will not be there", /background:#fff/.test(doc) &&
   !doc.includes("var(--"));
// Row text goes straight into markup here, so the whole document is checked against the vocabulary it is
// built from: an element outside this list can only have arrived from a blurb, a name or an install command
// that `esc()` missed. Stronger than looking for a stray `<` in a cell, which would not see one in a title.
const VOCAB = new Set(["html", "head", "meta", "title", "style", "body", "h1", "p", "a", "table",
  "thead", "tbody", "tr", "th", "td", "code", "br"]);
const strayTags = [...new Set([...doc.matchAll(/<\/?([a-zA-Z][a-zA-Z0-9]*)/g)].map(m => m[1]))]
  .filter(t => !VOCAB.has(t.toLowerCase()));
const rawAmp = doc.match(/&(?!(amp|lt|gt|quot|#39);)\S{0,12}/);
ok("...and every cell escaped, this being row text going straight into markup",
   strayTags.length === 0 && !rawAmp, strayTags.join(",") + " " + JSON.stringify(rawAmp?.[0] || ""));
ok("...and one <tr> per project plus the header",
   (doc.match(/<tr>/g) || []).length === A.HITS.length + 1,
   (doc.match(/<tr>/g) || []).length + " for " + A.HITS.length);
// Both halves of the install column, because the flag can be off and a view can have no detected command.
ok("the install column appears only when the flag is on and some row has a command",
   A.hasCmds() === (/"index\.install_commands":\s*1/.test(html) && A.HITS.some(r => r.install)),
   String(A.hasCmds()));

// --- the filename, which is what somebody reads in a folder in a fortnight ---
const name = A.fileName("md");
ok("the file names the view rather than the site", /^atlas-projects-matching-agent-\d{4}-\d\d-\d\d\.md$/
   .test(name), name);
ok("...using nothing a filesystem or a shell argues with", /^[a-z0-9.-]+$/.test(name), name);
A.state.q = "a very long search phrase " + "x".repeat(300); A.state.shown = 600; A.render();
const long = A.fileName("html");
ok("a pasted essay in the search box does not become the filename",
   long.replace(/^atlas-/, "").replace(/-\d{4}-\d\d-\d\d\.html$/, "").length === 60, long);
A.state.q = ""; A.state.cat = ""; A.state.shown = 600; A.render();
ok("and with no filter on, the title is the honest one", A.viewTitle() === "Every project in the atlas",
   A.viewTitle());

// --- the dialog ---
A.state.q = "agent"; A.state.shown = 600; A.render();
G2("take").click();
ok("the Export chip opens the dialog modally", G2("exp").open === true);
ok("...saying how many projects and which view, before anything is downloaded",
   /<b>[\d,]+ projects<\/b>/.test(G2("expwhat").innerHTML) && /matching/.test(G2("expwhat").innerHTML),
   G2("expwhat").innerHTML);
ok("...with the shareable link already in the box", G2("expurl").value === A.shareURL(),
   G2("expurl").value);
ok("...and all three documents offered", [G2("expmd"), G2("exphtml"), G2("expprint")]
   .every(b => b.disabled === false));
G2("expdone").click();
ok("...and Done closes it", G2("exp").open === false);
A.state.q = "zzqqxx-nothing-matches-this"; A.state.shown = 600; A.render();
ok("the empty view really is empty", A.HITS.length === 0, String(A.HITS.length));
G2("take").click();
ok("on an empty view the three documents are refused, a header with no rows being no use",
   [G2("expmd"), G2("exphtml"), G2("expprint")].every(b => b.disabled === true));
ok("...but the link is still live, because a link to an empty view is still a link",
   G2("expurl").value.length > 0, G2("expurl").value);
G2("expdone").click();

// --- the print swap, which is three things the stylesheet cannot reach ---
A.state.q = "agent"; A.state.view = "cards"; A.state.shown = 24;
document.documentElement.dataset.theme = "dark";
A.render();
A.printOn();
ok("printing switches to the table: a card is a box with a screenshot in it",
   A.state.view === "table", A.state.view);
ok("...and to the light theme, browsers printing backgrounds off by default",
   document.documentElement.dataset.theme === "light", document.documentElement.dataset.theme);
ok("...and expands the page, paper having no Show more button",
   A.state.shown >= A.HITS.length && A.HITS.length > 24, A.state.shown + " for " + A.HITS.length);
ok("...and writes the link to this view onto the sheet",
   G2("printurl").textContent === A.shareURL(), G2("printurl").textContent);
const restoring = A.PRINT.shown;
A.printOn();
ok("a second beforeprint does not overwrite what is waiting to be restored",
   A.PRINT.shown === restoring, A.PRINT.shown + " vs " + restoring);
A.printOff();
ok("afterprint puts the view back", A.state.view === "cards" && A.state.shown === 24,
   A.state.view + " / " + A.state.shown);
ok("...and the theme the reader chose", document.documentElement.dataset.theme === "dark",
   document.documentElement.dataset.theme);
ok("...and the rows the expansion added come off the page again", order().length === 24,
   String(order().length));
A.printOff();
ok("a stray afterprint changes nothing", A.state.view === "cards" && A.PRINT === null);
// The palette is the keyboard route to every control on this page, and a chip it cannot reach is a chip
// half the readers of this site do not have. Searched for rather than read off the empty palette, which
// caps at 40 entries and so does not show the Page group's own controls until something is typed.
ok("the command palette offers the export",
   A.palItems("export").some(i => i.group === "Page" && /Export/.test(i.label)),
   A.palItems("export").map(i => i.group + ":" + i.label).join(" | "));

globalThis.location.href = "";
globalThis.location.hash = "";
STORE.clear(); A.loadSaved();
A.state.list = new Set(); A.state.saved = false; A.state.q = ""; A.state.cat = "";
A.state.shown = 600; A.render();
...
// The other half of that pair, and the half that had a real defect. A comment closed twice -- `*/` in the
// middle of it and again at the end -- strips cleanly up to the first terminator and leaves the rest of the
// prose in the stylesheet as tokens. CSS error recovery then discards everything up to the next `}`, which
// is the *next rule*, so a paragraph of English in a comment silently deleted `.shared{display:none}` and
// put a shared-list strip on every page that had no shared list. Nothing else on the page noticed: the
// braces still balanced, the file still parsed, and every harness was green.
const closers = (html.match(/\*\//g) || []).length;
ok("...and no stray comment terminator, which is what a comment closed twice leaves behind",
   closers === 0, closers + " left, first at " + around(html.indexOf("*/")));

// So the stylesheet is read back as rules rather than as text, and every selector checked for being one.
// A selector cannot contain the `--` dash this file's prose uses, a backtick, or a sentence's full stop; a
// comment that lost its terminator produces all three, and produces them *in the selector position*, which
// is the one place the browser will accept them without complaint.
const selectors = cssRules(styles).map(r => r.sel);
const prose = selectors.filter(s => / -- /.test(s) || s.includes("`") || /\w\. [a-z]/.test(s));
ok("the stylesheet is rules and not prose", prose.length === 0,
   prose.map(s => JSON.stringify(s.slice(0, 60))).join(" | "));
ok("...and there are enough of them for that to have been worth checking",
...
for (const rule of [".takechip{display:none}", ".savechip{display:none}", ".shared{display:none}"])
  ok("the stylesheet keeps " + rule, styles.includes(rule));
